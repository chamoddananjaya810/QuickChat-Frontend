...
const AppColors = {
  background: { gradient1: '#f0f9ff', gradient2: '#e0f2fe', gradient3: '#bae6fd' },
  text: { primary: '#334155', secondary: '#64748b' },
  input: { background: '#ffffff', border: '#e2e8f0', placeholder: '#94a3b8', text: '#334155' },
  button: { primaryBg: '#3b82f6', primaryText: '#ffffff', primaryHover: '#2563eb' },
...
export default function SignInScreen() {
  ...
  useEffect(() => {
    const defaultCountry: ExtendedCountry = {
      cca2: 'LK', name: 'Sri Lanka', flag: '🇱🇰', callingCode: ['94'],
    } as ExtendedCountry;
  ...
  return (
    <View style={styles.flexOne}>
      ...
      <LinearGradient
        colors={[AppColors.background.gradient1, AppColors.background.gradient2, AppColors.background.gradient3]}
          ...
          >
            ...
            <View>
              ...
              <View style={styles.logoContainer}>
                <Image 
                  source={{ uri: "https://placehold.co/128x112/bae6fd/334155?text=Logo&font=sans" }} 
                  style={styles.logo} 
                  resizeMode="contain" 
                />
              </View>
              ...
              {/* Input Fields */}
              <View>
                <Pressable onPress={() => setShowPicker(true)} style={[styles.inputContainer, styles.countryPickerButton]}>
                  <Text style={styles.countryText}>
                      {country?.flag ? String(country.flag) : '🇱🇰'} {country?.name ? String(country.name) : 'Sri Lanka'}
                  </Text>
                  <Ionicons name="caret-down" size={16} color={AppColors.text.primary} />
                </Pressable>
                
                <CountryPicker
                    countryCode={country?.cca2 as CountryCode || 'LK'}
                    withFilter withFlag withCallingCode visible={showPicker}
                    onClose={() => setShowPicker(false)}
                    onSelect={(c) => {
                        const selectedCallingCode = Array.isArray(c.callingCode) ? c.callingCode[0] : c.callingCode;
                        setCountry(c);
                ...
                <View style={[styles.inputContainer, styles.passwordContainer]}>
                    ...
                    <TouchableOpacity onPress={() => setIsPasswordVisible(!isPasswordVisible)}>
                        <Ionicons name={isPasswordVisible ? "eye-off" : "eye"} size={24} color={AppColors.text.secondary} />
                    </TouchableOpacity>
                </View>
              </View>
            </View>
...
const styles = StyleSheet.create({
    ...
    headerTitle: { fontSize: 32, fontWeight: 'bold', color: AppColors.text.primary },
    headerSubtitle: { fontSize: 16, color: AppColors.text.secondary, marginTop: 8 },
    inputContainer: {
    ...
    countryText: { color: AppColors.text.primary, fontSize: 16, fontWeight: '500' },
    ...
    buttonText: { color: AppColors.button.primaryText, fontSize: 18, fontWeight: 'bold' },
    footer: { flexDirection: 'row', justifyContent: 'center', marginTop: 24 },
    footerText: { fontSize: 14, color: AppColors.text.secondary },
    footerLink: { fontSize: 14, color: AppColors.button.primaryBg, fontWeight: 'bold' }
});
